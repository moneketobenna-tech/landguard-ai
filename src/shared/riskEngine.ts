...
import { RiskLevel, ScanResult } from './constants';
...
const PRICE_PATTERNS = [
  /\$\s*\d{1,3}\s*(k|thousand)/i,  // $50k format
  /\$\s*[\d,]+/,                    // Standard price
  /price reduced/i,
  /negotiable/i,
  /obo/i,
...
export function extractPageContent(): PageContent {
  ...
  return {
    title,
    description: description.substring(0, 10000), // Limit for performance
    price,
    imageCount,
 ...
 */
function analyzeContent(content: PageContent): RiskFlag[] {
 ...
 */
function generateRecommendations(flags: RiskFlag[], riskLevel: RiskLevel): string[] {
  ...
  // Always add these for any listing
  if (riskLevel !== "low") {
 ...
 */
export function scanPage(additionalInfo?: { phone?: string; email?: string; agent?: string }): ScanResult {
...
/**
 * Scan arbitrary text content (for popup manual entry)
 */
